...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for c in summary.columns:
        if not c.is_numeric and c.non_null > 0:
            unique_share = c.unique / c.non_null
            if c.unique >= 20 and unique_share > 0.5:
                high_cardinality_cols.append(c.name)
    ...
    for c in summary.columns:
        if c.is_numeric and c.non_null > 0:
            # восстановить долю нулей можно через missing + уникальные
            # но корректнее считать при наличии df, поэтому используем эвристику
            if c.min == 0 and c.mean is not None and c.mean < 0.3 * c.max:
